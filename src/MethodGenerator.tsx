...
export function MethodGenerator() {
  ...
  const handleGenerateAndDownload = async () => {
    ...
    try {
      const disciplineLessons: DisciplineLessons = { discipline, lessons};

      const response = await fetch("/api/method-generator", {
        method: "POST",
  ...
  return (
    <div className="mt-8 mx-auto w-full text-left flex flex-col gap-4">
      <div className="bg-[#1a1a1a] border-2 border-[#fbf0df] rounded-xl p-3 font-mono">
        <label className="block text-[#fbf0df] font-bold mb-2">Discipline:</label>
        <input
          type="text"
          value={discipline}
      ...
      {lessons.length > 0 && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h2 className="text-[#fbf0df] font-mono text-lg font-bold">Added Lessons:</h2>
            <button
          ...
          {discipline && (
            <div className="text-[#fbf0df] font-mono font-bold mb-2">Discipline: {discipline}</div>
          )}
      ...
      >
        ...
        <textarea
          ...
          placeholder={isDragging ? "Drop text file here..." : "Lesson text (or drop a .txt file here)"}
          required
        />
        <button
          type="submit"
