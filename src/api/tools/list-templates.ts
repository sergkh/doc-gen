...
export function registerListTemplates(server: McpServer) {
  server.registerTool(
    "list_templates",
    {
      ...
      Перед генерацією шаблона переконайся що у курсу є теми, література та результати.
      Використовуй якщо необхідно сформувати або згенерувати документ. Для формування повного URL шаблону візьми базовий URL MCP сервера без шляху й додай вміст поля url`,
      annotations: {
        idempotentHint: true,
        readOnlyHint: false
      },
