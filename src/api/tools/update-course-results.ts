...
export function registerUpdateCourseResults(server: McpServer) {
  server.registerTool(
    ...
    {
      description:
        "Оновлює результати активної дисципліни за номерами окремо для кожного типу: ЗК (загальні компетентності), СК (спеціальні компетентності), РН (результати навчання). " +
        "Результат ІК (інтегральна компетентність) додається автоматично (він один і обов'язковий). Якщо не вказано явно, підбери результати сам, у відповідності до того чи вони відповідають темам дисципліни." +
